  ...
  const statCards = [
    {
      ...
      value: balance,
      icon: Scale,
      color: balance >= 0 ? 'text-teal-600 dark:text-teal-400' : 'text-red-500 dark:text-red-400',
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={barData} barCategoryGap="30%">
              ...
              <YAxis
                ...
                axisLine={false}
                tickLine={false}
                tickFormatter={(v: number) => `KD ${v.toFixed(0)}`}
                width={72}
              />
              <Tooltip
                formatter={(v: number) => formatKWD(v)}
                contentStyle={{
                  background: 'var(--tw-prose-body, #1f2937)',
                  border: 'none',
                  borderRadius: '12px',
                  fontSize: '12px',
                }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                      {tx.merchant || tx.categories?.name || 'Transaction'}
                    </p>
