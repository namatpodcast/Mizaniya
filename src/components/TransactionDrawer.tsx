...
export default function TransactionDrawer({ open, onClose, onSaved, editTx, categories }: Props) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    if (file.size > MAX_BYTES) { toast('File must be under 10 MB', 'error'); return }
    setNewFile(file)
    setNewFilePreview(file.type.startsWith('image/') ? URL.createObjectURL(file) : null)
  }

  const clearNewFile = () => { setNewFile(null); setNewFilePreview(null); if (fileInputRef.current) fileInputRef.current.value = '' }
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    if (removeExisting) attachmentUrl = null
    if (newFile) {
      const url = await uploadFile(editTx?.id ?? crypto.randomUUID())
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 z-50 flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{editTx ? 'Edit transaction' : 'Add transaction'}</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"><X size={18} className="text-gray-500" /></button>
        </div>
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div className="flex rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
              {(['expense','income'] as const).map((t) => (
                <button key={t} type="button" onClick={() => setType(t)}
                  className={`flex-1 py-2.5 text-sm font-medium transition-colors capitalize ${type === t ? (t === 'income' ? 'bg-teal-600 text-white' : 'bg-red-500 text-white') : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="text" value={merchant} onChange={(e) => setMerchant(e.target.value)} placeholder="e.g. Lulu Hypermarket"
              className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          ...
          <div>
            ...
            {showExisting && (
              <div className="mb-3 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                {isImage(existingUrl!) ? <img src={existingUrl!} alt="Attachment" className="w-full max-h-40 object-cover" /> : (
                  <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 dark:bg-gray-800">
                    <FileText size={20} className="text-gray-400 shrink-0" />
                    <a href={existingUrl!} target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline truncate">View attachment</a>
                  </div>
                )}
                <div className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <button type="button" onClick={() => setRemoveExisting(true)} className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1"><XCircle size={13} /> Remove</button>
                </div>
              </div>
            )}
            {newFile && (
              <div className="mb-3 rounded-xl border border-indigo-200 dark:border-indigo-800 overflow-hidden">
                {newFilePreview
                  ? <img src={newFilePreview} alt="Preview" className="w-full max-h-40 object-cover" />
                  : <div className="flex items-center gap-3 px-4 py-3 bg-indigo-50 dark:bg-indigo-900/20"><FileText size={20} className="text-indigo-500 shrink-0" /><span className="text-sm text-gray-700 dark:text-gray-300 truncate">{newFile.name}</span></div>}
                <div className="flex items-center justify-between px-3 py-2 bg-indigo-50 dark:bg-indigo-900/20 border-t border-indigo-200 dark:border-indigo-800">
                  <span className="text-xs text-indigo-600 dark:text-indigo-400">{(newFile.size / 1024).toFixed(0)} KB</span>
                  <button type="button" onClick={clearNewFile} className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1"><XCircle size={13} /> Remove</button>
                </div>
              </div>
            )}
            {!newFile && (
              <button type="button" onClick={() => fileInputRef.current?.click()}
                className="w-full flex flex-col items-center gap-2 py-5 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-indigo-400 dark:hover:border-indigo-600 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                <div className="flex gap-2 text-gray-400 group-hover:text-indigo-500 transition-colors"><ImageIcon size={18} /><Paperclip size={18} /></div>
                <span className="text-sm text-gray-500 dark:text-gray-400 group-hover:text-indigo-500 transition-colors">{showExisting ? 'Replace attachment' : 'Upload image or file'}</span>
                <span className="text-xs text-gray-400 dark:text-gray-500">JPG, PNG, WEBP, GIF, PDF, TXT · max 10 MB</span>
              </button>
            )}
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={isRecurring} onChange={(e) => setIsRecurring(e.target.checked)} className="w-4 h-4 rounded accent-indigo-600" />
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex gap-3">
          {editTx && (
            <button type="button" onClick={handleDelete} disabled={deleting}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${confirmDelete ? 'bg-red-600 text-white hover:bg-red-700' : 'border border-red-300 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950'}`}>
              <Trash2 size={15} />{confirmDelete ? 'Confirm delete' : 'Delete'}
            </button>
          )}
          <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Cancel</button>
          <button onClick={handleSave} disabled={saving || uploading} className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white transition-colors">
